...
//        properties.setDatagramSize(1024);
        ...
        //add plug
        oscP5.plug(this, "toServer", "/toServer");
...
void mousePressed() {
        OscMessage myMessage = new  OscMessage("/toClient");
...
void draw() {
        ...
        text("[server]", 50, height/2-32);
        text("from : "+clientName, 10, height/2);
        if(clientValue >= 0){
                text("value : " + clientValue, 10, height/2+32);
        }else{
...
void toServer(String clientName, int value){
